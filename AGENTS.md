...
Before non-trivial work, read and follow [AGENT_CONTRACT.md](AGENT_CONTRACT.md),
then [SKILL.md](SKILL.md). For standard/release work and any goal, blocker,
checkpoint, concurrency, or evidence operation, also read
[references/workflow-contract.md](references/workflow-contract.md).

The workspace-local managed block below is the Codex checkpoint registration.
Claude Code must not execute it; Claude Code uses the separately owned block in
`CLAUDE.md`. Do not hand-edit either managed block.

<!-- save-work-status:managed-begin v6 -->
...
At turn start resolve this workspace root as <workspace-root>, then run: "E:\rayman\software\AI\RaymanCodingSkill\.agent-checkpoints\runtime\save-work-status-585de90e807fa5bc735303c0b2e4551655dcfbe918e844def5c4f922a9e77764.exe" auto --workspace "<workspace-root>" --vault-path "E:\rayman\software\AI\RaymanCodingSkill\.agent-checkpoints\status.sqlite3" --agent codex --event turn-start --scope workspace
