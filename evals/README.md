...
- `control`: neither; PATH entries exposing a `rayman` command (including Windows `.cmd`/`.bat` wrappers) are removed from the tool child process. On Windows, `cmd` rewrites a command line before tokenizing it, so the evaluator normalizes the same way — it removes caret escapes and resolves `%VAR%` against the exact environment the child would receive — and refuses the command if a resulting spelling names a `rayman` executable. A reference that names rayman (`%RAYMAN%`) and substring/replace syntax that could synthesize one (`%VAR:~i,j%`, `%VAR:a=b%`) are refused as unresolvable. Tokenization uses the delimiter set measured character-by-character against a real `cmd` rather than an assumed one, so `,rayman`, `@rayman`, and `rayman/x` are all recognized. Delayed expansion (`!VAR!`) is **refused, not modeled**: it is off by default, so its only command-line-reachable form carries an explicit `cmd /V` opt-in, and a command is refused only when that switch and a `!` both appear. Ordinary `cmd` syntax such as `echo %CD%`, `if %ERRORLEVEL% neq 0 …`, `for /f %i in (…)`, or `echo Done!` is not by itself evidence of anything and runs normally.

  This guard is deliberately **not** a complete model of `cmd`, and the source comments no longer claim it is. Known accepted gaps: a lone `%` cannot be reduced to a literal because its value only exists at runtime; the *contents* of a script the agent writes and then invokes are never inspected, so a batch file may enable delayed expansion and assemble any name it likes; and any route that reaches rayman without naming it in the command text is invisible here. It is one layer — combined with PATH removal, a fail-closed CWD scan before every spawn, and a rescan after the agent finishes — and it is an integrity guard, not an OS sandbox.
