...
if ($first -ne $second) { throw 'normalizer is not byte-stable on repeated execution' }
...
if ($repaired -ne $stable) { throw 'normalizer changes an already normalized file' }
Write-Output 'RAYMAN_EVAL_ORACLE_PASS self-invalidating-gate-v2'
