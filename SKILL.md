...
- Downloading software, materials, models, datasets, dependencies, documentation, or other artifacts must prefer resume-capable mechanisms first, such as `curl -C -`, `wget -c`, `aria2c -c`, HTTP Range/Accept-Ranges handling, or PowerShell BITS / `Start-BitsTransfer`. Record cache/temp or partial-file handling evidence for resumable transfers; if the server or tool does not support resume, record the unsupported reason before using a non-resumable fallback.
...
- When a feature, declaration, command, API, or claim is retired, tests that only prove that retired surface have no current-behavior purpose. `rayman assets scan` must cascade those test cases and matching feature-coverage `test_anchors` into retirement candidates when it can isolate them, and `rayman assets cleanup --apply` may remove isolated obsolete Rust test cases plus their orphan feature-coverage anchors before deleting the now-unreferenced retired asset. If the test case is already gone, cleanup may remove the pure orphan feature-coverage anchor by itself. If a stale test reference cannot be isolated to one test case or mixes current and retired proofs, success must stay blocked until the test is manually deleted, rewritten to prove current behavior, or explicitly exempted.
