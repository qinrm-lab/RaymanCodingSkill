...
- **Quality surface** — machine-readable maintainability findings from the project map; `map quality --check` fails only on error-level gaps. Multi-source Cargo and pyproject packages without indexed tests are blocking; unsupported ecosystems stay advisory. Strict/release always promote the built-in `large_file` and `high_fan_in` warnings. `.RaymanCodingSkill/quality.json` can only add blocking kinds or declare exact `(path, kind)` exemptions with a non-empty reason and a positive `reviewed_max` limit; it cannot clear those defaults. Growth beyond the reviewed metric restores normal warnings or blocking; see [quality policy](docs/QUALITY_POLICY.md).
- **Goal contract** — capture caller-authored `must`/`should` requirements, a baseline-bound plan, hierarchical work packages, source-bound non-authoritative progress receipts, validation receipts that cover the real delta, repeated-stable authority proof, and a fingerprint-bound review record for high-priority changes. These hashes do not prove equivalence to the host conversation or an independent reviewer identity. A pure zero-delta audit uses the authority-only `--workspace-snapshot` scope; it is rejected before the gate starts if the goal baseline has any real delta and never substitutes for `--changed` coverage. A lane ledger distinguishes scoped-writer lanes (which reject writes outside their allowlist) from advisory read-only and final-reviewer lanes (zero-write review brackets that reject every workspace change for their duration), while pending work records agent/human/external ownership.
...
- **Managed temp** — operator scratch and explicit CLI leases stay under `.RaymanCodingSkill/tmp/`. Native Windows validation children instead use the separately configured external host-temp roots so their cleanup authority never contains a workspace; every physical lease is still manifest-owned, probed, identity-revalidated, and released before evidence is written. `temp status` reports the workspace-local tree only.
- **Windows execution** — ordinary edits and tests use the native Windows sandbox. The installed Codex global execution backend handles authorized local commits, opted-in workflow state and registered file installation through protected, bound requests. Parallel writers use separate Git worktrees; see [execution setup](docs/GLOBAL_CODEX_EXECUTION.md).
...
The version declared in `Cargo.toml` binds exact authority/typed-proof semantics, versioned gate inputs, the source-fresh signed-asset chain, the delegated activation bundle, lossless recovery behavior, read-only probe opt-in, compact Goal projections, protected eval oracles, and the complete first-party source-rule-execution-gate-test lifecycle to `rayman-cli-contract-v18` and the exact running CLI version. It retains the signed-update worker, install receipt, safe `ensure-current`, and identity-only rebind boundaries while rejecting a fresh exact-floor replay and a same-version different install tuple. Keep these claims separate: `check --profile release` proves workspace strict-quality only; `doctor --check` proves installed binary/PATH/skill-bundle/activation identity and any explicitly required execution-context match; the versioned update worker is separately receipt- and signature-bound; release handoff additionally requires locked fresh-source rebuilds of both binaries. The exact release, update, and test-source procedures live in [docs/RELEASE_CONTRACT.md](docs/RELEASE_CONTRACT.md), [docs/UPDATE_CONTRACT.md](docs/UPDATE_CONTRACT.md), and [docs/TEST_TRACEABILITY.md](docs/TEST_TRACEABILITY.md).
...
Installation never scans for or rewrites activation contracts in other workspaces. Because each activation binds the exact canonical skill, its delegated agent/workflow bundle, and the running CLI identity, a successful upgrade can intentionally leave a previously activated workspace `invalid` until it is explicitly rebound there. Run `rayman workspace status` in each workspace. `rayman workspace rebind --yes` is eligible only for an existing complete seven-field binding, or a legacy six-field binding, whose only defects are skill/bundle hash, CLI contract, or CLI version identity drift and whose current complete bundle matches the bundle embedded in the running CLI; it updates only those identity scalars under the shared activation lock while preserving `skill_file`, comments, ordering, quoting, and line endings apart from inserting the missing bundle field during legacy migration. It refuses orphan, deactivated, malformed, wrong-skill, untrusted/missing-resource, unsafe-path, and path-change cases. Use `workspace activate --skill-file ... --yes` for a new activation or an intentional canonical-path change. When Rayman is explicitly invoked for a non-read-only task, perform an eligible rebind and continue that original task; a read-only request remains read-only and reports the recovery command instead. The Stop Hook cannot infer intent, so safely rebindable drift is allowed to end without forcing a state write only after normal goal/frontier checks; an unfinished goal or structurally invalid activation still blocks.
...
rayman workspace inspect [--probe-writes] # read-only by default; Git optional locks are disabled
...
# One-way and non-authoritative: invalid completed history only; current goals must have no pending work or open lanes/required packages, and every trusted archive route must fail.
...
goal-bound human solution package with `boundary_class=operator_pause`. Rendering
that package permits the Stop guard to end the turn while the Goal remains
unfinished; it is never success evidence or an invented external blocker.
...
`rayman state audit` is a read-only hygiene report. It names the v2 state entries it accepts, any retired entries, the recursive temp footprint, and crash-leaked atomic-write remnants. Structural `clean` remains separate from `operationally_healthy`: large byte/entry counts and safe-but-unresolved recovery remnants produce explicit non-blocking warnings without authorizing deletion. `--check` exits nonzero when retired state, an audit error, or a traversal error exists; it deliberately performs no migration or deletion. Review the report and obtain explicit exact-target approval before removing or migrating state.

Checkpoint manifests are v3 integrity records with activation provenance and `standard|recovery_only` purpose; v2 manifests are rejected rather than reinterpreted. `salvage-save` remains available when activation is invalid, but its recovery-only snapshot never becomes default `latest` or completion evidence. Restoring it requires an explicit ID, `--allow-recovery-only`, and a repaired active contract. Saves take a cross-process workspace/store lock; failed/crashed staging, partial snapshots, and recovery-only snapshots are never automatically deleted. An explicit `keep` policy rotates only verified complete standard snapshots. `checkpoint verify` is read-only. Restore validates the complete manifest first, then durably replaces each destination file through same-directory staging (`flush`/`fsync` + atomic rename). It is a journaled all-or-nothing transaction: every source is staged and verified and every existing destination is backed up before the first file is published, and a failure during publication restores the backups in reverse order. A crash leaves a recorded transaction that the next `save`/`restore` resolves; it never deletes extra workspace files.
...
For focused development before installation, use the commands below. The versioned
Cargo command table in `scripts/repository-quality.ps1` is shared by both gates;
`scripts/read-repository-quality.ps1` validates that table for both consumers:
...
cargo test --manifest-path evals/Cargo.toml --locked --all-targets
cargo deny --manifest-path evals/Cargo.toml check --config evals/deny.toml
```

CI (`.github/workflows/ci.yml`) defaults to `contents: read`, pins every third-party Action to an immutable commit, and grants `contents: write` only to the protected tag-only `signed-release` job. Root, traceability, and eval test matrices run on Windows, Linux, and macOS so the explicit non-Linux-Unix cases have a real execution target. Installer runtime tests remain hosted Windows x64 and Ubuntu/Linux x64; the Linux ARM64 leg is only a Rust cross-target `cargo check` and must never be reported as an ARM64 runtime PASS. CI also verifies Rust 1.97.1, self-dogfoods context refresh + strict quality + release readiness, and verifies clean-source release identity on the runtime platforms without a conflicting global `RUSTFLAGS`. Separate jobs exercise `cargo package`, `cargo install`, and enforce a real 75% line threshold for the root Cargo workspace, including the source-side `xtask`, rather than the standalone eval harness. Dependency policy runs on every change, with an additional scheduled weekly advisory refresh. `evals/` runs fmt/clippy/unit tests, both host-exec rejection guards, and an offline mock provenance smoke; no real-model credential or quality claim is involved.
