...
- **Owner Mode** — the agent keeps working through locally knowable implementation, repair, risk checks, and re-audit. Frontier reports separate execution from consultation: a deferred question stays out of transient progress output, while a ready question becomes one deterministic workspace aggregate. Only the current Codex `Stop` event can observe an exact aggregate; it does not create a durable claim that the user saw or read it. Background continuation requires an explicitly authorized isolated mechanism. The hook asks the same goal/frontier contracts before a turn may end, so an assistant cannot replace `finish --goal` with a reassuring final message.
...
- **Goal contract** — capture `must`/`should` requirements, a baseline-bound plan, hierarchical work packages, source-bound non-authoritative progress receipts, validation receipts that cover the real delta, repeated-stable authority proof, and fingerprint-bound review for high-priority changes. A lane ledger distinguishes scoped-writer lanes (which reject writes outside their allowlist) from advisory read-only and final-reviewer lanes (zero-write review brackets that reject every workspace change for their duration), while pending work records agent/human/external ownership.
...
`2.10.0` binds the upgrade-rebind surface to `rayman-cli-contract-v16` and the exact running CLI version. It retains the v2.9 identity-only `workspace rebind` boundary and adds goal-bound write-ahead plan publication, snapshot-bound `prepare` readiness, token-bound Windows profile diagnostics, capability-bound pending packages, and exact current-event Stop observation. Keep these claims separate: `check --profile release` proves workspace strict-quality only; `doctor --check` proves installed binary/PATH/SKILL/activation identity and any explicitly required execution-context match; release handoff additionally requires a locked fresh-source rebuild. The exact contract and release procedure live in [docs/RELEASE_CONTRACT.md](docs/RELEASE_CONTRACT.md).
