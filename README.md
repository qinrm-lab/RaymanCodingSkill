...
rayman workspace ensure-current [--yes] # report currency; --yes only reuses eligible rebind
rayman workspace deactivate --yes
rayman update status            # offline/read-only user update preference and cache
rayman update check             # explicit fixed-source network check; no cache/install write
rayman update poll              # due default notification; trusted worker only after separate consent
