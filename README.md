...
`2.9.0` binds the explicit upgrade-rebind surface to `rayman-cli-contract-v15` and the exact running CLI version. It retains the v2.8 resumable long-task behavior and adds an identity-only `workspace rebind` whose target bytes must match the canonical `SKILL.md` embedded in the running CLI. Keep these claims separate: `check --profile release` proves workspace strict-quality only; `doctor --check` proves installed binary/PATH/SKILL/activation identity only; release handoff additionally requires a locked fresh-source rebuild. The exact contract and release procedure live in [docs/RELEASE_CONTRACT.md](docs/RELEASE_CONTRACT.md).
