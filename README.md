...
- **Goal contract** — capture `must`/`should` requirements, a baseline-bound plan, hierarchical work packages, source-bound non-authoritative progress receipts, validation receipts that cover the real delta, repeated-stable authority proof, and fingerprint-bound review for high-priority changes. A lane ledger distinguishes advisory read-only, scoped writer, and final-reviewer concurrency while pending work records agent/human/external ownership.
...
`2.8.0` binds the resumable long-task surface to `rayman-cli-contract-v14` and the exact running CLI version. It adds probed pytest leases, recovery-only checkpoint salvage, hierarchical work packages/progress receipts, compact resume summaries, and source-bound lane ledgers without weakening final validation authority. Keep these claims separate: `check --profile release` proves workspace strict-quality only; `doctor --check` proves installed binary/PATH/SKILL/activation identity only; release handoff additionally requires a locked fresh-source rebuild. The exact contract and release procedure live in [docs/RELEASE_CONTRACT.md](docs/RELEASE_CONTRACT.md).
