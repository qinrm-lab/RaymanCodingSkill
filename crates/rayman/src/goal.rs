...
use crate::file_io::{is_link_or_reparse, read_json, write_json};
...
pub fn replacement_authority_effective_command(
    ...
    if rebind.schema != MAINTENANCE_CYCLE_REBIND_SCHEMA
        ...
        || rebind.current_value.is_empty()
        || rebind.current_value.contains('\\')
        || !rebind
            .current_value
            .ends_with("-maintenance-review-cycle.json")
    {
    ...
    let relative = Path::new(&rebind.current_value);
    if relative.is_absolute()
        || relative
            .components()
            .any(|component| !matches!(component, Component::Normal(_)))
    {
        bail!("maintenance cycle rebind current_value 不是规范 workspace-relative 路径");
    }
    let normalized = relative
        .components()
        .filter_map(|component| match component {
        ...
        .collect::<Vec<_>>()
        .join("/");
    if normalized != rebind.current_value {
        bail!("maintenance cycle rebind current_value 不是唯一规范文本形式");
    }
