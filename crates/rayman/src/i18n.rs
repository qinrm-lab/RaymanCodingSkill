...
pub const AUTHORED_MESSAGE_TEMPLATES: &[&str] = &[
    ...
    "repository authority goal 缺少 receipt-era baseline",
    "repository replacement authority dependency binding 无效",
    "repository replacement authority 缺少 receipt-era dependency binding",
    "repository replacement authority binding 与 captured authority gate closure 不一致",
    "activation skill capture identity/bytes 结构不一致",
    "activation skill 与同轮 workspace capture 不一致: {key}",
    "captured maintenance cycle rebind artifact 缺失",
    "captured maintenance cycle rebind 文件 hash 已漂移",
    "context index 在 captured refresh 发布后发生变化",
    "current captured workspace snapshot 缺失",
    "current goal 缺少开工 baseline；不能作为当前成功证据",
    "goal baseline 或 captured workspace snapshot 与 fingerprint 不匹配",
    "lifecycle-only replacement 只接受 current-v3 receipt policy",
    "maintenance artifact 路径别名冲突: {previous} vs {key}",
    "maintenance cycle rebind 文件 hash 已漂移: {key}",
    "readiness goal 文件名与内部 id 不匹配: file={name} id={}",
    "readiness goals 含重复 goal id: {}",
    "readiness goals 成员必须是普通非链接文件: {name}",
    "readiness goals 目录含未知成员: {name}",
    "readiness goals 目录含非 UTF-8 成员: {:?}",
    "readiness goals 目录成员在捕获期间发生变化",
    "readiness goals 目录成员枚举失败: {}",
    "readiness workspace capture 包含重复路径: {key}",
    "ready context 缺少 verified index",
    "receipt_integrity_v3 lifecycle proof 缺少 workspace identity",
    "receipt_integrity_v3 receipt workspace identity 与 lifecycle proof 不一致",
    "skill_file capture 与 activation 记录路径不匹配",
    "skill_file capture 结构无效",
    "skill_file 不可安全读取 {}: readiness capture 缺少对应观察",
    "validation receipt workspace identity 与当前工作区不匹配",
    "当前 captured workspace snapshot 缺失",
    "当前 workspace capture 包含重复路径: {}",
    "无法从 captured workspace 重算 repository replacement authority gate binding: {error:#}",
    "无法枚举 readiness goals 目录: {}",
    "无法规范化 cargo metadata workspace root: {}",
    "无法规范化 cargo metadata {label}: {}",
    "无法规范化 readiness workspace: {}",
    "无法解析 captured .RaymanCodingSkill/quality.json",
    "无法解析 captured context/index.json",
    "无法解析 captured goal JSON",
    "无法解析 captured pending.json",
    "无法解析刚发布的 captured context/index.json",
    "无法读取 lifecycle-only authority goal snapshot: {error}",
    "无法读取 lifecycle-only authority goal snapshot: {}",
    "无法读取 readiness goals 成员元数据: {name}",
    "项目地图 capture 与 context entry 不匹配: {} (size {} != {} or sha256 {} != {})",
    "项目地图 capture 缺少已验证文件: {}",
...
const TEMPLATE_FRAGMENT_CATALOG: &[(&str, &str)] = &[
    ...
    (
        "repository replacement authority binding 与 captured authority gate closure 不一致",
        "the repository replacement authority binding does not match the captured authority-gate closure",
    ),
    (
        "activation skill capture identity/bytes 结构不一致",
        "the activation skill capture identity/bytes structure is inconsistent",
    ),
    (
        "activation skill 与同轮 workspace capture 不一致: {key}",
        "the activation skill does not match the same-cycle workspace capture: {key}",
    ),
    (
        "captured maintenance cycle rebind artifact 缺失",
        "the captured maintenance-cycle rebind artifact is missing",
    ),
    (
        "captured maintenance cycle rebind 文件 hash 已漂移",
        "the captured maintenance-cycle rebind file hash has drifted",
    ),
    (
        "context index 在 captured refresh 发布后发生变化",
        "the context index changed after captured refresh publication",
    ),
    (
        "current captured workspace snapshot 缺失",
        "the current captured workspace snapshot is missing",
    ),
    (
        "current goal 缺少开工 baseline；不能作为当前成功证据",
        "the current goal lacks a starting baseline and cannot serve as current success evidence",
    ),
    (
        "goal baseline 或 captured workspace snapshot 与 fingerprint 不匹配",
        "the goal baseline or captured workspace snapshot does not match the fingerprint",
    ),
    (
        "lifecycle-only replacement 只接受 current-v3 receipt policy",
        "lifecycle-only replacement accepts only the current-v3 receipt policy",
    ),
    (
        "maintenance artifact 路径别名冲突: {previous} vs {key}",
        "maintenance artifact path aliases conflict: {previous} vs {key}",
    ),
    (
        "maintenance cycle rebind 文件 hash 已漂移: {key}",
        "the maintenance-cycle rebind file hash has drifted: {key}",
    ),
    (
        "readiness goal 文件名与内部 id 不匹配: file={name} id={}",
        "readiness goal filename does not match its internal id: file={name} id={}",
    ),
    (
        "readiness goals 含重复 goal id: {}",
        "readiness goals contain a duplicate goal id: {}",
    ),
    (
        "readiness goals 成员必须是普通非链接文件: {name}",
        "readiness goal members must be regular non-link files: {name}",
    ),
    (
        "readiness goals 目录含未知成员: {name}",
        "the readiness goals directory contains an unknown member: {name}",
    ),
    (
        "readiness goals 目录含非 UTF-8 成员: {:?}",
        "the readiness goals directory contains a non-UTF-8 member: {:?}",
    ),
    (
        "readiness goals 目录成员在捕获期间发生变化",
        "a readiness goals directory member changed during capture",
    ),
    (
        "readiness goals 目录成员枚举失败: {}",
        "failed to enumerate readiness goals directory members: {}",
    ),
    (
        "readiness workspace capture 包含重复路径: {key}",
        "the readiness workspace capture contains a duplicate path: {key}",
    ),
    (
        "ready context 缺少 verified index",
        "ready context is missing a verified index",
    ),
    (
        "receipt_integrity_v3 lifecycle proof 缺少 workspace identity",
        "the receipt_integrity_v3 lifecycle proof lacks workspace identity",
    ),
    (
        "receipt_integrity_v3 receipt workspace identity 与 lifecycle proof 不一致",
        "the receipt_integrity_v3 receipt workspace identity does not match the lifecycle proof",
    ),
    (
        "skill_file capture 与 activation 记录路径不匹配",
        "the skill_file capture does not match the activation's recorded path",
    ),
    (
        "skill_file capture 结构无效",
        "the skill_file capture structure is invalid",
    ),
    (
        "skill_file 不可安全读取 {}: readiness capture 缺少对应观察",
        "skill_file cannot be read safely {}: readiness capture lacks the corresponding observation",
    ),
    (
        "validation receipt workspace identity 与当前工作区不匹配",
        "the validation receipt workspace identity does not match the current workspace",
    ),
    (
        "当前 captured workspace snapshot 缺失",
        "the current captured workspace snapshot is missing",
    ...
    (
        "当前 workspace capture 包含重复路径: {}",
        "the current workspace capture contains a duplicate path: {}",
    ),
    (
        "无法从 captured workspace 重算 repository replacement authority gate binding: {error:#}",
        "unable to recompute the repository replacement authority-gate binding from the captured workspace: {error:#}",
    ),
    (
        "无法枚举 readiness goals 目录: {}",
        "unable to enumerate the readiness goals directory: {}",
    ),
    (
        "无法规范化 cargo metadata workspace root: {}",
        "unable to canonicalize the cargo metadata workspace root: {}",
    ),
    (
        "无法规范化 cargo metadata {label}: {}",
        "unable to canonicalize cargo metadata {label}: {}",
    ),
    (
        "无法规范化 readiness workspace: {}",
        "unable to canonicalize the readiness workspace: {}",
    ),
    (
        "无法解析 captured .RaymanCodingSkill/quality.json",
        "unable to parse captured .RaymanCodingSkill/quality.json",
    ),
    (
        "无法解析 captured context/index.json",
        "unable to parse captured context/index.json",
    ),
    (
        "无法解析 captured goal JSON",
        "unable to parse captured goal JSON",
    ),
    (
        "无法解析 captured pending.json",
        "unable to parse captured pending.json",
    ),
    (
        "无法解析刚发布的 captured context/index.json",
        "unable to parse the just-published captured context/index.json",
    ),
    (
        "无法读取 lifecycle-only authority goal snapshot: {error}",
        "unable to read the lifecycle-only authority goal snapshot: {error}",
    ),
    (
        "无法读取 lifecycle-only authority goal snapshot: {}",
        "unable to read the lifecycle-only authority goal snapshot: {}",
    ),
    (
        "无法读取 readiness goals 成员元数据: {name}",
        "unable to read readiness goal member metadata: {name}",
    ),
    (
        "项目地图 capture 与 context entry 不匹配: {} (size {} != {} or sha256 {} != {})",
        "project-map capture does not match the context entry: {} (size {} != {} or sha256 {} != {})",
    ),
    (
        "项目地图 capture 缺少已验证文件: {}",
        "project-map capture is missing a verified file: {}",
    ),
    (
        "lifecycle-only authority {label} 不得晚于 replacement_authority.recorded_at",
    ...
    (
        "repository authority binding 与 authority goal baseline 不一致",
        "the repository authority binding does not match the authority goal baseline",
    ),
    (
        "repository authority goal 缺少 receipt-era baseline",
        "the repository authority goal has no receipt-era baseline",
    ),
    (
        "repository replacement authority dependency binding 无效",
        "the repository replacement authority dependency binding is invalid",
    ),
    (
        "repository replacement authority 缺少 receipt-era dependency binding",
        "the repository replacement authority has no receipt-era dependency binding",
    ),
    (
        "authority receipt invocation hash 无效",
    ...
    (
        "orphan restore transaction 没有 journal 却仍存有备份文件，无从判断该回滚哪些目标；已保留供人工恢复。恢复步骤：检查该目录 backups/ 子目录中的原件、取回仍需要的文件，再删除整个目录以解除对 save/restore 的阻塞：{}",
        "orphan restore transaction has no journal yet still holds backup files, so there is no way to tell which targets to roll back; it was kept for manual recovery. Recovery steps: inspect the originals under that directory's backups/, take back whatever you still need, then delete the whole directory to unblock save/restore: {}",
    ),
    (
        "orphan restore transaction 自动回滚不完整，已保留并拒绝继续: {}",
    ...
    (
        "回滚目标已被第三方修改，拒绝覆盖: {}。原件仍在该 transaction 的 backups/ 子目录中；取回仍需要的内容后删除整个 transaction 目录即可解除对 save/restore 的阻塞，salvage-save 不受阻塞",
        "the rollback target was modified by a third party; refusing to overwrite: {}. The originals are still under that transaction's backups/ directory; take back whatever you still need, then delete the whole transaction directory to unblock save/restore. salvage-save is never blocked by it",
    ),
    (
        "完整 checkpoint manifest 含有跳过项或错误记录",
...
const MESSAGE_FRAGMENT_CATALOG: &[(&str, &str)] = &[
    ...
    (
        "`rayman autosave` 已退役；自动保存使用独立的 save-work-status 技能，手动快照与恢复使用 `rayman checkpoint`",
        "`rayman autosave` is retired; use the separate save-work-status skill for automatic recovery and `rayman checkpoint` for manual snapshots and restore",
    ),
];
...
mod tests {
    ...
    fn known_fragments_still_translate_framework_text() {
        ...
        assert_eq!(
            localize_line_for(
                "`rayman autosave` 已退役；自动保存使用独立的 save-work-status 技能，手动快照与恢复使用 `rayman checkpoint`".into(),
                ActiveLanguage::En,
                false,
            ),
            "`rayman autosave` is retired; use the separate save-work-status skill for automatic recovery and `rayman checkpoint` for manual snapshots and restore"
