...
- Treat unbound `rayman check` as workspace health only. Task completion
  requires a goal-bound gate, but that gate proves only the exact caller-authored
  Goal. Claiming that it completes the user's request additionally requires a
  fresh explicit comparison with the actual request; Goal text and reviewer
  names are not transcript or independent-identity attestations. Release
  completion additionally requires installation, repository audit, and
  source-fresh proof.
...
- An explicit user pause or shutdown request has precedence over Owner Mode.
  Preserve unfinished work as a non-completion operator-pause package and
  permit the turn to stop; never reinterpret the pause as success or as an
  external blocker.
...
`AGENTS.md`, `SKILL.md`, and `CLAUDE.md` are adapters, not independent policy
copies. Any change to this shared contract must keep their `AGENT_CONTRACT:
