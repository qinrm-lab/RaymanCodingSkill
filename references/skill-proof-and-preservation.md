...
- Executed tests and validation commands have higher authority than AI judgment. Primary AI, auxiliary AI, research output, and host subagent summaries are advisory until backed by current evidence, and they cannot countermand a failed or missing validation result.
...
- Untested certainty is worse than explicit unknown: if the only support is AI/model confidence, report `unknown`, `assumption`, or `advisory`; if validation failed, report `blocked` until a later successful validation command clears the failure.
