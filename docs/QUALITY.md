...
- `obsolete_asset_retirement`: review, refactor, and feature replacement work must include obsolete-asset inventory, replacement/current behavior evidence, docs/config/tests synchronization, `rayman assets status`, and `rayman audit` evidence before success. Obsolete assets default to retirement/deletion; retained assets require an explicit compatibility or audit reason, expiry date, and exclusion from current-behavior context evidence.
