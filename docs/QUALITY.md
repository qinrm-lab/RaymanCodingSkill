...
- `resumable_download_preference`: downloads of software, materials, models, datasets, dependencies, documentation, or artifacts must prefer resume-capable transfer mechanisms such as `curl -C -`, `wget -c`, `aria2c -c`, HTTP Range/Accept-Ranges, or PowerShell BITS. Evidence must show the resume-capable command/protocol plus cache or partial-file handling, or an explicit server/tool unsupported reason before any non-resumable fallback.
