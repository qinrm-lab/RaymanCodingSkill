...
4. The public command surface `workspace` (including path-safe Git-aware `workspace inspect`), `context`, `goal` (including orthogonal execution/consultation frontier states, evidence-bound background continuation, monotonic plan extension, and repeated-stable authority validation), workspace-health and goal-bound `check`, locked `prepare` plus `finish`, `map`, `assets`, `temp`, read-only `state audit --check`, `checkpoint` (including integrity-verifying `checkpoint verify`), `autosave`, and `doctor`; the v2.7 workflow surface adds global `--language auto|zh-CN|en`, locale detection, strict UTF-8/Unicode-path coverage, and locale-invariant JSON, uses runtime report label `rayman-cli-contract-v13`, and records both contract and version in activation. This surface is proven by the crate's behavioral tests (`crates/rayman/tests/cli.rs` plus in-crate unit tests) on every CI platform, not by scraping `--help` text in the release verifier.
