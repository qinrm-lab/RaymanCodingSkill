...
3. The canonical repository `SKILL.md`, its delegated `AGENT_CONTRACT.md` and `references/workflow-contract.md` resources, and the exact `skill_sha256` plus aggregate `bundle_sha256` recorded in the target workspace's `.RaymanCodingSkill/workspace_skill.yaml`.
4. The public command surface `codex-hook`, `workspace` (including path-safe Git-aware read-only `workspace inspect` with explicit write probes, trusted identity-only `workspace rebind`, and `workspace ensure-current`), activation-exempt `update status|check|configure|poll`, `context`, `goal` (including goal-bound write-ahead plan publication, capability-bound pending packages, operator-pause packages, deterministic client-neutral `rayman.human-boundary-aggregate.v1` rendering, gate-checked frontier states, monotonic plan extension, hierarchical packages, compact list/current projections, non-authoritative progress receipts, source-bound lane ledgers, and repeated-stable authority validation), workspace-health and goal-bound `check`, snapshot-bound locked `prepare` plus `finish`, `map`, `assets`, `temp` (including probed pytest leases), structurally clean versus operationally healthy read-only `state audit --check`, lossless `checkpoint` (including activation-exempt recovery-only salvage and integrity-verifying `checkpoint verify`), and execution-context-aware `doctor`; the v2.12 surface retains multilingual UTF-8/Unicode and v2.8 long-task behavior under runtime label `rayman-cli-contract-v18`. Activation records the exact contract and version. Update notification, signed-release installation, source freshness, and workspace activation are distinct claims; see [UPDATE_CONTRACT.md](UPDATE_CONTRACT.md). Native completion observation is adapter-owned: a Codex Stop match is event-local, Claude Code does not execute that hook, and neither creates a durable delivery/read receipt. Progress/lane/recovery-only records are explicitly non-authoritative and cannot satisfy final validation. Behavioral tests cover this surface on the configured CI platforms rather than scraping `--help` text in the release verifier; only a successful run bound to the exact source supplies release evidence.
...
- `rayman doctor --check` proves the installed binary/PATH/workspace skill-and-delegated-contract-bundle **identity tuple**. It explicitly does not prove that the artifact was rebuilt from the current source.
...
# doctor requires the workspace's complete seven-field activation to match the
# current skill bundle. Use `workspace status`, then an eligible
# `workspace rebind --yes`; never hand-edit either recorded hash.
...
That repository-local rewrite is not a registry or migration of other workspaces. The installer never scans for or automatically changes their `.RaymanCodingSkill/workspace_skill.yaml` files. Since activation binds the exact canonical skill, its delegated contract bundle, and CLI contract/version, a successful upgrade can intentionally make an existing external workspace `invalid`. In each such workspace, first run `rayman workspace status`. `rayman workspace rebind --yes` accepts only an existing complete seven-field binding, or a legacy six-field binding, whose defects are limited to skill/bundle hash or CLI identity drift and whose current complete bundle matches the bundle embedded in the running CLI; under the shared activation lock it updates only the identity scalar values while preserving all other contract bytes apart from inserting the missing bundle field during the one-way legacy migration. Orphan, deactivated, malformed, wrong-skill, untrusted/missing-resource, unsafe-path, and path-change cases fail closed and require review or an explicit `workspace activate --skill-file ... --yes`. Explicit Rayman use for a non-read-only task performs an eligible rebind and then resumes the original task; read-only work reports the command without rewriting state. The intent-blind Stop Hook never forces that write, but it still blocks an unfinished Owner Mode goal before allowing eligible drift to end.
...
It includes root and evals fmt/Clippy/tests/dependency policy, `cargo package`
and `cargo install` smoke, context refresh, strict quality, release readiness,
the `state audit --check` gate plus a report-only `assets` scan, and this
installed release contract. Goal-bound closeout then executes the exact
source-bound Rust gate `cargo run --locked --manifest-path xtask/Cargo.toml --
repository-gate` as the separate repeated authority step. The convenient
...
Scheduled `autosave` is retired. Its compatibility command only returns migration
guidance; it is not a release capability. Manual checkpoints and old snapshot
restoration remain supported, while automatic recovery belongs to SaveStatus.
