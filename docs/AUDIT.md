...
4. A real `cargo llvm-cov` line threshold for the root Cargo workspace (the shipped `rayman` crate plus source-side `xtask`), compiled with the exact isolated MSRV Cargo/rustc and matching `llvm-tools-preview`. This threshold does not claim coverage for the standalone eval harness.
