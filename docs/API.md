...
Goal and research responses append `evidence_status`, `claim_ledger`, `unknowns`, `assumptions`, and `blockers` without removing the persisted record fields. `verified` requires current workspace path evidence, a recorded successful validation command, current goal/session/context state, or an existing evidence artifact. Auxiliary AI, cached summaries, memory, research output, and confidence remain advisory only.
...
Asset retirement responses return `workspace_path`, `generated_at`, `state_path`, `controller_scope`, `ignored_roots`, `blockers`, `candidates`, `retired_present`, `exemptions`, `cleanup_plan`, `detected_references`, `records`, `required_actions`, and `source_policy`.
