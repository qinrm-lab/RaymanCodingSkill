...
`rayman subagent dispatch --task "<task>" --path <path>` creates a durable dispatch record under `.RaymanCodingSkill/subagents/dispatches/` for host-request, non-overlapping scope, and local worktree-lane coordination. Add `--create-worktree` when a writable lane needs a real isolated checkout; Rayman then creates `.RaymanCodingSkill/worktrees/<dispatch-id>` with `git worktree add --detach` and records the outcome. `rayman subagent reconcile` is the fail-closed closeout check: it blocks unclosed dispatch files, missing primary review, conflicts, scope overlap, out-of-bound changed paths, and failed/unavailable requested worktrees. An unavailable host lane should be closed with `record`, `result --status failed`, and `review --verdict not-used` evidence instead of being omitted.

Trace/eval/replay is the repeatability layer for modern coding-agent workflows. `rayman trace record`, `rayman trace status`, and `rayman trace replay` write deterministic JSONL process events, `rayman eval dataset run --dataset <path> --grader contains` writes grader reports, and the `trace_eval` gate requires trace parse/replay health plus at least one passed eval report. These artifacts prove process repeatability and regression coverage only; they do not replace per-requirement current-file evidence.

`rayman workflow learn --name <name> --evidence <item>` stores a repeated workflow candidate under `.RaymanCodingSkill/workflows/`. `rayman workflow promote --id <id>` promotes only after replay, eval dataset, and readiness-gate evidence are present. This prevents a useful-looking repeated process from becoming a current pattern based on memory, confidence, or a single unvalidated run.
...
rayman subagent dispatch --task "docs and gate sync" --path docs --read-only --max-lanes 2
...
rayman trace record --kind validation -m "cargo test -p rayman-core"
rayman trace replay
rayman eval dataset run --dataset .RaymanCodingSkill/evals/dataset.json --grader contains
rayman workflow learn --name "repeatable gate repair" --evidence "rayman trace replay"
