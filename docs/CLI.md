...
`rayman assets status` reports workspace-local obsolete asset retirement state from `.RaymanCodingSkill/assets/retirement.json`, including controller scope, ignored scan roots, blockers, candidates, retired assets still present, compatibility exemptions, cleanup plan, detected references, required actions, and source policy.

`rayman assets scan` rereads current files, recomputes stale docs/config/tests/CLI/API references to recorded obsolete assets, ignores non-current roots such as `.git`, `target`, build caches, and `.RaymanCodingSkill/tmp`, and writes the refreshed workspace-local retirement state.

`rayman assets cleanup --apply` deletes only retirement-candidate files already registered in `.RaymanCodingSkill/assets/retirement.json`, only when they still resolve inside the current workspace and have no current references. It does not delete unknown files, active `raymancodingskill` or `rayman.exe` assets, or arbitrary user data. Directory cleanup is blocked until an explicit per-file manifest is available; there is no bare recursive delete path.
...
`rayman evidence check [--scope workspace|goal|session|research] [--goal-id <id>] [--include-advisory] [--format text|json]` reports evidence-backed claim status. JSON output includes `evidence_status`, `claim_ledger`, `unknowns`, `assumptions`, `blockers`, and `required_actions`. `verified` requires a current workspace path, recorded successful validation command, current goal/session/context state, or an existing evidence artifact; auxiliary AI, cached summaries, memory, research output, and confidence remain advisory.

`rayman coverage status` checks `config/feature_coverage.yaml`, verifies documentation anchors, implementation anchors, test anchors, public CLI commands, public API endpoints, and UI contract markers, and prints a JSON report by default. Use `--strict` to require entries marked `strict_validation: true` to carry current passed `validation_records` with `updated_at`, `evidence_path`, exact command text, passed status, and configured `evidence_contains` text. Use `--check` in gates; it enables strict validation. Use `--format markdown --output docs/FEATURE_COVERAGE.md` to refresh the human-readable matrix, and combine it with `--check` to fail when the matrix document is stale.
